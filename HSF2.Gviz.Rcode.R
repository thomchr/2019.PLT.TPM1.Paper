...
chr <- 'chr6' #this is how everything downstream can be generic. Can just change the 'chr#' for each
gen <- 'hg19'
loc <- 122721510
...
pdf("HSF2.10kb.locus.atac.pdf")
ub <- loc + 10000
lb <- loc - 10000
plotTracks(list(ot, grtrack, HSC, CD34, MEP, Ery, gtrack), chromosome = chr, from = lb, to = ub, type = "histogram", scale = 0.25, labelPos = 'below', collapseTranscripts = 'meta', transcriptAnnotation = 'gene_name')
dev.off() 

pdf("HSF2.10kb.locus.pdf")
ub <- loc + 10000
lb <- loc - 10000
...
#pdf("HSF2.70kb.SNPsGRMarks.locus.pdf")
...
pdf("HSF2.100kb.SNPs.GeneRegiononly.locus.pdf")
...
pdf("HSF2.1Mb.GRonly.noannotation.locus.pdf")
