...
chr <- 'chr11' #this is how everything downstream can be generic. Can just change the 'chr#' for each
gen <- 'hg19'
loc <- 308065
...
pdf("IFITM2.100kb.wWGS.locus.pdf")
ub <- loc + 100000
lb <- loc - 100000
...
pdf("IFITM2.10kb.wWGS.locus.pdf")
ub <- loc + 10000
lb <- loc - 10000
plotTracks(list(ot2, ot, grtrack, gtrack), chromosome = chr, from = lb, to = ub, type = "histogram", scale = 0.25, labelPos = 'below', collapseTranscripts = 'meta', transcriptAnnotation = 'gene_name')
dev.off() 
...
pdf("IFITM2.100kb.WGS.locus.pdf")
ub <- loc + 100000
lb <- loc - 100000
...
pdf("IFITM2.10kb.locus.atac.pdf")
ub <- loc + 10000
lb <- loc - 10000
plotTracks(list(ot, grtrack, HSC, CD34, MEP, Ery, gtrack), chromosome = chr, from = lb, to = ub, type = "histogram", scale = 0.25, labelPos = 'below', collapseTranscripts = 'meta', transcriptAnnotation = 'gene_name')
dev.off() 

pdf("IFITM2.10kb.locus.pdf")
ub <- loc + 10000
lb <- loc - 10000
...
#pdf("IFITM2.70kb.SNPsGRMarks.locus.pdf")
...
pdf("IFITM2.100kb.SNPs.GeneRegiononly.locus.pdf")
...
pdf("IFITM2.1Mb.GRonly.locus.pdf")
...
pdf("IFITM2.1Mb.GRonly.noannotation.locus.pdf")
