...
chr <- 'chr10' #this is how everything downstream can be generic. Can just change the 'chr#' for each
gen <- 'hg19'
loc <- 65016174
...
pdf("JMJD1C.100kb.wWGS.locus.pdf")
ub <- loc + 100000
lb <- loc - 100000
...
pdf("JMJD1C.10kb.wWGS.locus.pdf")
ub <- loc + 10000
lb <- loc - 10000
plotTracks(list(ot2, ot, grtrack, gtrack), chromosome = chr, from = lb, to = ub, type = "histogram", scale = 0.25, labelPos = 'below', collapseTranscripts = 'meta', transcriptAnnotation = 'gene_name')
dev.off() 
...
pdf("JMJD1C.100kb.WGS.locus.pdf")
ub <- loc + 100000
lb <- loc - 100000
...
#pdf("JMJD1C.70kb.SNPsGRMarks.locus.pdf")
...
pdf("JMJD1C.100kb.SNPs.GeneRegiononly.locus.pdf")
...
pdf("JMJD1C.1Mb.GRonly.locus.pdf")
...
pdf("JMJD1C.1Mb.GRonly.noannotation.locus.pdf")
