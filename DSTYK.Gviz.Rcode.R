...
chr <- 'chr1' #this is how everything downstream can be generic. Can just change the 'chr#' for each
gen <- 'hg19'
loc <- 205110929
...
pdf("DSTYK.100kb.wWGS.locus.pdf")
ub <- loc + 100000
lb <- loc - 100000
...
pdf("DSTYK.100kb.PltRBCproxies.locus.pdf")
ub <- loc + 100000
lb <- loc - 100000
plotTracks(list(ot2, ot, grtrack, HSC, CD34, MEP, Ery, gtrack), chromosome = chr, from = lb, to = ub, type = "histogram", scale = 0.25, labelPos = 'below', collapseTranscripts = 'meta', transcriptAnnotation = 'gene_name')
dev.off()

pdf("DSTYK.1Mb.locus.pdf")
ub <- loc + 1000000
lb <- loc - 1000000
...
#pdf("DSTYK.70kb.SNPsGRMarks.locus.pdf")
...
pdf("DSTYK.100kb.SNPs.GeneRegiononly.locus.pdf")
...
pdf("DSTYK.1Mb.GRonly.locus.pdf")
...
pdf("DSTYK.1Mb.GRonly.noannotation.locus.pdf")
