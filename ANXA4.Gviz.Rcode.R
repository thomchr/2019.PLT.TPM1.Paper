...
chr <- 'chr2' #this is how everything downstream can be generic. Can just change the 'chr#' for each
gen <- 'hg19'
loc <- 70043775
...
pdf("ANXA4.100kb.wWGS.locus.pdf")
ub <- loc + 100000
lb <- loc - 100000
...
pdf("ANXA4.10kb.locus.atac.pdf")
ub <- loc + 10000
lb <- loc - 10000
plotTracks(list(ot, grtrack, HSC, CD34, MEP, Ery, gtrack), chromosome = chr, from = lb, to = ub, type = "histogram", scale = 0.25, labelPos = 'below', collapseTranscripts = 'meta', transcriptAnnotation = 'gene_name')
dev.off() 

pdf("ANXA4.10kb.locus.pdf")
ub <- loc + 10000
lb <- loc - 10000
...
#pdf("ANXA4.70kb.SNPsGRMarks.locus.pdf")
...
pdf("ANXA4.100kb.SNPs.GeneRegiononly.locus.pdf")
...
pdf("ANXA4.1Mb.GRonly.locus.pdf")
...
pdf("ANXA4.1Mb.GRonly.noannotation.locus.pdf")
