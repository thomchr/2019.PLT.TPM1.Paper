...
chr <- 'chr8' #this is how everything downstream can be generic. Can just change the 'chr#' for each
gen <- 'hg19'
loc <- 56684796
...
pdf("TMEM68.100kb.wWGS.locus.pdf")
ub <- loc + 100000
lb <- loc - 100000
...
pdf("TMEM68.10kb.wWGS.locus.pdf")
ub <- loc + 10000
lb <- loc - 10000
plotTracks(list(ot2, ot, grtrack, gtrack), chromosome = chr, from = lb, to = ub, type = "histogram", scale = 0.25, labelPos = 'below', collapseTranscripts = 'meta', transcriptAnnotation = 'gene_name')
dev.off() 
...
pdf("TMEM68.100kb.WGS.locus.pdf")
ub <- loc + 100000
lb <- loc - 100000
...
pdf("TMEM68.10kb.locus.atac.pdf")
ub <- loc + 10000
lb <- loc - 10000
plotTracks(list(ot, grtrack, HSC, CD34, MEP, Ery, gtrack), chromosome = chr, from = lb, to = ub, type = "histogram", scale = 0.25, labelPos = 'below', collapseTranscripts = 'meta', transcriptAnnotation = 'gene_name')
dev.off() 

pdf("TMEM68.10kb.locus.pdf")
ub <- loc + 10000
lb <- loc - 10000
...
#pdf("TMEM68.70kb.SNPsGRMarks.locus.pdf")
...
pdf("TMEM68.100kb.SNPs.GeneRegiononly.locus.pdf")
...
pdf("TMEM68.1Mb.GRonly.locus.pdf")
...
pdf("TMEM68.1Mb.GRonly.noannotation.locus.pdf")
