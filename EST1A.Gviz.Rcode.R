...
chr <- 'chr17' #this is how everything downstream can be generic. Can just change the 'chr#' for each
gen <- 'hg19'
loc <- 1988377
...
pdf("EST1A.100kb.wWGS.locus.pdf")
ub <- loc + 100000
lb <- loc - 100000
...
pdf("EST1A.10kb.locus.atac.pdf")
ub <- loc + 10000
lb <- loc - 10000
plotTracks(list(ot, grtrack, HSC, CD34, MEP, Ery, gtrack), chromosome = chr, from = lb, to = ub, type = "histogram", scale = 0.25, labelPos = 'below', collapseTranscripts = 'meta', transcriptAnnotation = 'gene_name')
dev.off() 

pdf("EST1A.10kb.locus.pdf")
ub <- loc + 10000
lb <- loc - 10000
...
#pdf("EST1A.70kb.SNPsGRMarks.locus.pdf")
...
pdf("EST1A.100kb.SNPs.GeneRegiononly.locus.pdf")
...
pdf("EST1A.1Mb.GRonly.locus.pdf")
...
pdf("EST1A.1Mb.GRonly.noannotation.locus.pdf")
