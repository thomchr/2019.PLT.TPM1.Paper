...
LASSO <- read.table(file="AstlePLT.LASSO.cat.Validation.forROCR", header=T) 
#dim(LASSO)
#lapply(LASSO, class)
#str(LASSO)

GWAVA <- read.table(file="AstlePLT.GWAVA.cat.Validation.forROCR", header=T)
#dim(GWAVA)
#str(GWAVA)

CADD <- read.table(file="AstlePLT.CADD.cat.Validation.forROCR", header=T)
#dim(CADD)
#str(CADD)

DeepSea <- read.table(file="AstlePlatelet.DeepSEA.Validation.forROCR", header=T)
#dim(DeepSea)
#str(DeepSea)
...
pdf("200311.AstlePLT.Validation.ROCR.LASSO.GWAVA.performance.pdf")
plot( perf1, main="Astle PLT Validation Index vs 100 ctrls each", col='black') #LASSO
...
legend("bottomright", legend=c("LASSO - AUC 0.771", "GWAVA - AUC 0.836", "CADD - AUC 0.596", "DeepSEA - AUC 0.550"), col=c("black", "red", "purple", "green"), lty=1:2, cex=0.8)
...
pdf("200311.AstlePLT.Validation.ROCR.LASSO.GWAVA_fprstop0.2_performance.pdf")
plot(perf1, main="Astle PLT Validation vs 54,000 random snp147 ctrls", xlim=c(0,0.2), col='black') #LASSO
#plot( perfa, add = TRUE, col='orange') #LASSOvs1328000 ctrls
plot(perf4, add = TRUE, col='red') #GWAVA red
#plot( perfb, add = TRUE, col='black') #GWAVA1328000ctrls
