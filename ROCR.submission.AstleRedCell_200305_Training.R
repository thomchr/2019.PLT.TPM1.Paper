...
LASSO <- read.table(file="AstleRedCell.LASSO.cat.Training.forROCR", header=T) 
dim(LASSO)
#lapply(LASSO, class)
str(LASSO)

GWAVA <- read.table(file="AstleRedCell.GWAVA.cat.Training.forROCR", header=T) 
dim(GWAVA)
str(GWAVA)

CADD <- read.table(file="AstleRedCell.CADD.cat.Training.forROCR", header=T)
#dim(CADD)
#str(CADD)

DeepSea <- read.table(file="AstleRedCell.Training.DeepSea.forROCR", header=T)
#dim(DeepSea)
#str(DeepSea)
...
pdf("200311.AstleRedCell.Training.ROCR.LASSO.GWAVA.performance.pdf")
plot( perf1, main="Astle Red Cell Training Index vs 100 ctrls each", col='black') #LASSO
...
legend("bottomright", legend=c("LASSO - AUC 0.818", "GWAVA - AUC 0.740", "CADD - AUC 0.548", "DeepSEA - AUC 0.611"), col=c("black", "red", "purple", "green"), lty=1:2, cex=0.8)
...
pdf("200311.AstleRedCell.Training.ROCR.LASSO.GWAVA_fprstop0.2_performance.pdf")
plot(perf1, main="Astle Training Red Cell Index vs 10,000 random snp147 ctrls", xlim=c(0,0.2), col='black') #LASSO
plot(perf4, add = TRUE, col='red') #GWAVA red
