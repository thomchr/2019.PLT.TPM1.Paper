...
LASSO <- read.table(file="Gieger73.LASSO.forROCR", header=T)
dim(LASSO)
str(LASSO)

GWAVA <- read.table(file="Gieger73.GWAVA.forROCR", header=T)
dim(GWAVA)
str(GWAVA)

CADD <- read.table(file="Gieger73.CADD.forROCR", header=T)
dim(CADD)
str(CADD)

DeepSea <- read.table(file="Gieger73.DeepSea.forROCR", header=T)
dim(DeepSea)
str(DeepSea)
...
pdf("Gieger73.ROCR.LASSO.DeepSea.CADD.GWAVA.performance.pdf")
plot( perf, main="Gieger73 Training Index vs 500 random snp147 ctrls", col='black') #LASSO
...
legend("bottomright", legend=c("LASSO - AUC 0.82", "CADD - AUC 0.50", "GWAVA - AUC 0.78", "DeepSea - AUC 0.58"), col=c("black", "red", "purple", "green"), lty=1:2, cex=0.8)
dev.off()
