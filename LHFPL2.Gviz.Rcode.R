...
chr <- 'chr5' #this is how everything downstream can be generic. Can just change the 'chr#' for each
gen <- 'hg19'
loc <- 77779738
...
pdf("LHFPL2.100kb.wWGS.locus.pdf")
ub <- loc + 100000
lb <- loc - 100000
...
pdf("LHFPL2.10kb.wWGS.locus.pdf")
ub <- loc + 10000
lb <- loc - 10000
plotTracks(list(ot2, ot, grtrack, gtrack), chromosome = chr, from = lb, to = ub, type = "histogram", scale = 0.25, labelPos = 'below', collapseTranscripts = 'meta', transcriptAnnotation = 'gene_name')
dev.off() 
...
pdf("LHFPL2.100kb.WGS.locus.pdf")
ub <- loc + 100000
lb <- loc - 100000
...
pdf("LHFPL2.10kb.locus.atac.pdf")
ub <- loc + 10000
lb <- loc - 10000
plotTracks(list(ot, grtrack, HSC, CD34, MEP, Ery, gtrack), chromosome = chr, from = lb, to = ub, type = "histogram", scale = 0.25, labelPos = 'below', collapseTranscripts = 'meta', transcriptAnnotation = 'gene_name')
dev.off() 

pdf("LHFPL2.10kb.locus.pdf")
ub <- loc + 10000
lb <- loc - 10000
...
#pdf("LHFPL2.70kb.SNPsGRMarks.locus.pdf")
...
pdf("LHFPL2.100kb.SNPs.GeneRegiononly.locus.pdf")
...
pdf("LHFPL2.1Mb.GRonly.locus.pdf")
...
pdf("LHFPL2.1Mb.GRonly.noannotation.locus.pdf")
